<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('body').on('click', '#delete_classe', function(e){
            e.preventDefault();
            var id = $(this).data('id');
                $('.modal-title').text('Suppression de l\'école n°'+id);
                $('.delete_classe_modal').modal('show');
                $('#btn_delete_classe').val(id);
                $('#form')[0].reset();
        });

        $('#btn_delete_classe').click(function(){
            var id = $(this).val();
            $.ajax({
                type: 'DELETE',
            ...
            $('#form')[0].reset();
            $('.delete_classe_modal').modal('hide');
