...
function rendtableau_comm(datedebut,datefin,nom_reg,nom_comm,nom_fin){
  fetch('/repas/commune?start='+datedebut+'&end='+datefin+'&nom_reg='+nom_reg+'&nom_comm='+nom_comm+'&nom_fin='+nom_fin)
  ...
  .then(data => {
    ...
    document.getElementById('table_parcommune').textContent = '';
    
    myGrid.render(document.getElementById('table_parcommune'));
    myGrid.forceRender();
    ...
    const da = [[
      ...
      XLSX.writeFile(workbook, 'commune.xlsx');
    });
  })
}