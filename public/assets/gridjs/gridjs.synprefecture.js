...
function rendtableau_pref(datedebut,datefin,nom_pref,nom_fin){
  fetch('/repas/prefecture?start='+datedebut+'&end='+datefin+'&nom_pref='+nom_pref+'&nom_fin='+nom_fin)
  ...
  .then(data => {
    ...
    document.getElementById('table_parprefecture').textContent = '';
    
    myGrid.render(document.getElementById('table_parprefecture'));
    myGrid.forceRender();
    ...
    const da = [[
      ...
      XLSX.writeFile(workbook, 'prefecture.xlsx');
    });
  })
}